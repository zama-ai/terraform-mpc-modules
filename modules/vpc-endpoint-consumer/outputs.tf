...
output "partner_service_details" {
  ...
  value = [
    for service_name, endpoint in aws_vpc_endpoint.party_interface_endpoints : {
      service_name                 = service_name
      partner_region               = local.party_services_map[service_name].region
      partner_account_id           = local.party_services_map[service_name].account_id # Can be null
      ...
      created_kube_service         = local.party_services_map[service_name].create_kube_service
      ports                        = local.party_services_map[service_name].ports
    }
  ]
}
...
output "partner_connection_endpoints" {
  ...
  value = {
    for service_name, service in local.party_services_map : service_name => {
      # Primary connection methods
      vpc_interface_dns       = aws_vpc_endpoint.party_interface_endpoints[service_name].dns_entry[0].dns_name
      kubernetes_service_name = service.create_kube_service ? "mpc-node-${service.party_id}.${var.create_namespace && anytrue([for s in var.party_services : s.create_kube_service]) ? kubernetes_namespace.partner_namespace[0].metadata[0].name : var.namespace}.svc.cluster.local" : null
      custom_dns_name         = var.create_custom_dns_records ? "${service_name}.${var.dns_domain}" : null
      ...
      network_interface_ips = aws_vpc_endpoint.party_interface_endpoints[service_name].network_interface_ids
    }
  }
}
