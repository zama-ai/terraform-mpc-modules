...
locals {
  ...
  party_services_map = {
    for service in var.party_services : service.name => service
  }
...
resource "kubernetes_service" "party_services" {
  ...
  metadata {
    name      = "mpc-node-${each.value.party_id}"
    ...
    labels = merge({
      "app.kubernetes.io/name"      = "kms-${each.value.party_id}-core"
      "app.kubernetes.io/instance"  = "kms-${each.value.party_id}-core"
